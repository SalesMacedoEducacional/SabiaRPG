...
// Endpoint para alunos ativos baseado em logins reais
app.get('/api/alunos-ativos', async (req, res) => {
  try {
    console.log('Buscando estatísticas de alunos ativos baseado em logins reais');
    ...
    const ativos7DiasResult = await executeQuery(`
      SELECT COUNT(DISTINCT ul.usuario_id) as count
      FROM user_logins ul
      JOIN usuarios u ON ul.usuario_id = u.id
      JOIN matriculas m ON u.id = m.aluno_id
      WHERE u.papel = 'aluno' 
      AND m.escola_id = ANY($1)
      AND ul.login_timestamp >= NOW() - INTERVAL '7 days'
    `, [escolaIds]);
    ...
    const ativos30DiasResult = await executeQuery(`
      ...
      WHERE u.papel = 'aluno' 
      AND m.escola_id = ANY($1)
      AND ul.login_timestamp >= NOW() - INTERVAL '30 days'
    `, [escolaIds]);
    
    // Taxa de engajamento (alunos ativos últimos 7 dias / total de alunos)
    ...
    const ativos7Dias = parseInt(ativos7DiasResult.rows[0]?.count || '0');
    const ativos30Dias = parseInt(ativos30DiasResult.rows[0]?.count || '0');
    const totalAlunos = parseInt(totalAlunosResult.rows[0]?.count || '0');
    const taxaEngajamento = totalAlunos > 0 ? Math.round((ativos7Dias / totalAlunos) * 100) : 0;
    ...
    res.status(500).json({
      message: 'Erro ao buscar alunos ativos',
      error: error.message
    });
  }
});

// Endpoint para detalhes dos alunos ativos com filtros
app.get('/api/alunos-ativos/detalhes', async (req, res) => {
  try {
    const { periodo = '7', escola_id } = req.query;
    console.log(`Buscando detalhes de alunos ativos - período: ${periodo} dias`);
    ...
    if (escola_id) {
      ...
      escolaIds = escolasResult.rows.map(e => e.id);
    }
    
    const alunosAtivosResult = await executeQuery(`
      SELECT DISTINCT
        u.id,
        u.nome,
        u.email,
        ...
        MAX(ul.login_timestamp) as ultimo_login,
      ...
      WHERE u.papel = 'aluno' 
      AND m.escola_id = ANY($1)
      AND ul.login_timestamp >= NOW() - INTERVAL '${periodo} days'
      GROUP BY u.id, u.nome, u.email, e.nome, t.nome
      ORDER BY ultimo_login DESC
    `, [escolaIds]);
    
    res.json({
      message: 'Alunos ativos obtidos com sucesso',
      periodo: parseInt(periodo),
      alunos: alunosAtivosResult.rows
    });
    
  } catch (error) {
    console.error('Erro ao buscar detalhes dos alunos ativos:', error);
    res.status(500).json({
      message: 'Erro ao buscar detalhes dos alunos ativos',
      error: error.message
    });
  }
});
